...
export default function LargeProduct({ product, images }: { product: SelectProduct, images: SelectProductImage[] }) {
  ...
  return (
    <div className="grid grid-cols-3 px-3 gap-2 relative" >
      ...
      <p className="text-theme-dark-teal text-md font-medium self-center p-1 sm:justify-self-center">${product.price}</p>
      <AddToCart product={product} addToCart={function (product: SelectProduct): void {
        throw new Error('Function not implemented.');
      } } />
      <p className="text-md p-3 col-span-3 md:px-[75px]">{product.description}</p>
    </div>
  );
}